...
function FinalizarPedido({ onVoltar }) {
  ...
  async function handleConfirmar() {
    ...
    const observacaoTxt = observacao
      ? `%0A%F0%9F%93%9D *Observação:* ${observacao}`
      : "";
    ...
    const itensTxt = itens
      .map((i) => {
        ...
        if (i.adicionais?.length > 0) {
          const adicionaisTxt = i.adicionais
            .map((a) => `  ➕ ${a.quantidade}x ${a.nome}`)
            .join("%0A");
          linha += `%0A${adicionaisTxt}`;
        }
        return linha;
    })
    .join("%0A");
    ...
    <>
      <div className="max-w-2xl mx-auto px-4 py-6 pb-10">
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 mb-6">
          ...
          {itens.map((item) => (
            <div key={`${item.id}-${item.opcao}`} className="flex flex-col py-1 gap-0.5">
              ...
              {item.adicionais?.length > 0 && (
                <div className="flex flex-col gap-0.5 pl-3">
                  {item.adicionais.map((a) => (
                    <span key={a.id} className="text-zinc-500 text-xs">
                      + {a.quantidade}x {a.nome} (R$ {(a.preco * a.quantidade).toFixed(2).replace(".", ",")})
                    </span>
                  ))}
                </div>
              )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <textarea
            placeholder="Alguma observação? (ex: sem cebola, ponto bem passado...)"
            value={observacao}
            onChange={(e) => setObservacao(e.target.value)}
            rows={3}
            className="bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-white placeholder-zinc-500 focus:outline-none focus:border-yellow-400 resize-none"
          />
      ...
      {modalConfirmacao && (
        <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50 px-4 pb-6">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-5 w-full max-w-sm">
            <h3 className="text-white font-bold text-lg mb-4">Confirmar pedido?</h3>
            ...
            {itens.map((item) => (
              ...
              {taxaEntrega > 0 && (
                <div className="flex justify-between text-zinc-400 text-sm pt-1 mt-1 border-t border-zinc-800">
                  <span>Taxa de entrega</span>
                  <span>R$ {taxaEntrega.toFixed(2).replace(".", ",")}</span>
                </div>
              )}
              <div className="flex justify-between text-yellow-400 font-bold pt-2 mt-1 border-t border-zinc-800">
                <span>Total</span>
                <span>R$ {totalComEntrega.toFixed(2).replace(".", ",")}</span>
              </div>
            </div>
            ...
            <p className="text-zinc-400 text-sm mb-4">
              {tipoEntrega === "retirada" ? "🏠 Retirada no local" : tipoEntrega === "vila" ? `🏘️ Vila — ${endereco}` : `🛵 Fazenda: ${fazendaSelecionada}`}
              {" · "}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl font-bold bg-zinc-800 text-white hover:bg-zinc-700 transition"
              >
                Cancelar
              </button>
              <button
                onClick={handleConfirmar}
                className="flex-1 py-3 rounded-xl font-bold bg-yellow-400 text-black hover:bg-yellow-300 transition"
              >
                Confirmar ✓
              </button>
            </div>
